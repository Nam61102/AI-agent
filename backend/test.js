...
async function check() {
  ...
  const query = \SELECT e.*, m.text AS source_text, m.sender_jid, m.chat_jid,
      ...
      LEFT JOIN contacts sender ON sender.jid = m.sender_jid AND sender.account_jid = \
      LEFT JOIN contacts chat ON chat.jid = m.chat_jid AND chat.account_jid = \
      LEFT JOIN suggested_replies sr ON sr.source_message_id = m.id AND sr.status = 'pending' AND sr.account_jid = \
      WHERE e.account_jid = \
  ...
  console.log('Result count BEFORE dedup:', res.rows.length);
  process.exit(0);
}
check();
