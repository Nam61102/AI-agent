...
class WhatsAppService {
  ...
  private initSocket() {
    ...
    try {
      ...
      // Real-time WhatsApp web socket events
      this.socket.on('whatsapp:realtime_chats', (data: { chats: any[] }) => {
        if (data?.chats) {
          this.listeners.forEach(l => l.onRealtimeChats?.(data.chats));
        }
      });

      this.socket.on('whatsapp:new_message', (data: { jid: string; message: any }) => {
        if (data) {
          this.listeners.forEach(l => l.onNewMessage?.(data));
        }
      });

      this.socket.on('whatsapp:chat_messages', (data: { jid: string; messages: any[] }) => {
        if (data) {
  ...
  public async connect(): Promise<{ success: boolean; status: ConnectionStatus; requiresScan?: boolean }> {
    ...
    try {
      ...
      console.error('[WhatsAppService] Connection error:', err);
      this.updateStatus('ERROR');
      this.notifyError('Failed to connect to WhatsApp server.');
    }
  ...
  public async requestPairingCode(phoneNumber: string): Promise<{ success: boolean; code?: string; error?: string }> {
    ...
    try {
      ...
      return { success: false, error: err.message || 'Network error requesting pairing code' };
    }
  }
  ...
  private runMockConnectionFlow(): Promise<{ success: boolean; status: ConnectionStatus; requiresScan?: boolean }> {
    if (this.mockTimer) clearTimeout(this.mockTimer);
    ...
    return Promise.resolve({ success: true, status: 'CONNECTING', requiresScan: true });
  }
